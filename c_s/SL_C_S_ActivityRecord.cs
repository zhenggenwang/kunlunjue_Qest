...
function ShowFactionBaseInfo(%master, %content){
	if (%master == 6){  //是帮主
		KLJFactionWndGui_AddMember.setVisible(true);
		KLJFactionWndGui_AskList.setVisible(true);
		KLJFactionWndGui_ChangeReMark.setVisible(true);
		KLJFactionWndGui_KickOut.setVisible(true);
		KLJFactionWndGui_TransferLeader.setVisible(true);
		KLJFactionWndGui_SaveAffiche.setVisible(true);
		KLJFactionWndGui_LeaveFaction.setVisible(false);
		KLJFactionWndGui_DisbandFaction.setVisible(true);
		$g_ShowFactionMangerButton = 1;
	}
	else if (%master == 5){
		KLJFactionWndGui_AddMember.setVisible(true);
		KLJFactionWndGui_AskList.setVisible(true);
		...
		KLJFactionWndGui_KickOut.setVisible(true);
		KLJFactionWndGui_SaveAffiche.setVisible(true);
		KLJFactionWndGui_LeaveFaction.setVisible(true);
		KLJFactionWndGui_DisbandFaction.setVisible(false);
	}
	else
	{
		KLJFactionWndGui_LeaveFaction.setVisible(true);
	...
	%baseInfo = GetFactionBaseInfo(%content);
	//echo("%baseInfo===" @ %baseInfo);
	KLJFactionWndGui_FactionName.setText(Org_GetWord(%baseInfo, 0));			//帮派名称
	KLJFactionWndGui_FactionLV.setText(Org_GetWord(%baseInfo, 2));          //帮派等级
	KLJFactionWndGui_FactionExp.setPercentum(Org_GetWord(%baseInfo, 3));    //帮派经验	 
	KLJFactionWndGui_FactionExpNum.setText(Org_GetWord(%baseInfo, 1));	    //帮派经验
	KLJFactionWndGui_FactionMember.setText(Org_GetWord(%baseInfo, 6));      //帮派人数
	KLJFactionWndGui_FactionReputation.setText(Org_GetWord(%baseInfo, 1));  //帮派声望
	KLJFactionWndGui_FactionFund.setText(Org_GetWord(%baseInfo, 7));  //帮派资金
	KLJFactionWndGui_FactionActive.setText(Org_GetWord(%baseInfo, 8));      //帮派活跃
	KLJFactionWndGui_Affiche.setText(Org_GetWord(%baseInfo, 9));            //帮派公告
}

function GetFactionBaseInfo(%content){
	echo("content===" @ %content);
	%text  = Org_GetWord(%content, 0);//0 帮派名称
	%level = Org_GetWord(%content, 3);
	%text = %text TAB Org_GetWord(%content, 1) @ "/" @ Org_GetWord(%content, 2); //1 声望 /最大声望 现用于帮派经验显示
	%text = %text TAB %level @ "级";//2  帮派等级

	%Percentum = Org_GetWord(%content, 1)/Org_GetWord(%content, 2);
	%text = %text TAB %Percentum;//3 现用于帮派经验条显示
	%trade = Org_GetWord(%content, 4);
	if (%trade < 0)
		%text = %text TAB "0/0";                                    //4 跑商次数
	else
		%text = %text TAB %trade @ "/" @ Org_GetWord(%content, 5);  //4 跑商次数
	%text = %text TAB Org_GetWord(%content, 6);                     //5 帮主名称
	%text = %text TAB Org_GetWord(%content, 7) @ "/" @ Org_GetWord(%content, 8);	//6 人数/人数上限
	%text = %text TAB Org_GetWord(%content, 9);                                 	//7  帮派资金
	%text = %text TAB Org_GetWord(%content, 10) @ "/" @ Org_GetWord(%content, 11);//8  帮派活跃度
	%text = %text TAB Org_GetWord(%content, 12);																	//9 
	%text = %text TAB Org_GetWord(%content, 13); 																	//10
	%text = %text TAB Org_GetWord(%content, 14);
	%text = %text TAB Org_GetWord(%content, 15);
	%text = %text TAB Org_GetWord(%content, 16);
	%text = %text TAB Org_GetWord(%content, 17);
	%text = %text TAB Org_GetWord(%content, 18);
	%text = %text TAB Org_GetWord(%content, 19);
	return %text;
}